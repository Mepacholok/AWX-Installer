...
EOF
    ...
    kubectl wait --for=condition=Ready -n awx awx/awx-demo --timeout=900s
    
    # Get admin password
    local admin_password=$(kubectl get secret awx-demo-admin-password -o jsonpath="{.data.password}" -n awx | base64 --decode)
...
show_final_info() {
    ...
    if [[ $INSTALL_METHOD == "operator" ]]; then
        ...
        echo "- View logs: kubectl logs -f deployment/awx-demo -n awx"
